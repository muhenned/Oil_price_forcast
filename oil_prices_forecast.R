...
##   Since this part is time consuming, I have attached it is output in separate file.
...
fit_sutse <- out1[[opt]]
#save(fit_sutse,file="Sutse_output.RDATA")

load("Sutse_output.RDATA")
### fitted model
...
# A multivariate seaminggly unrelated Dynamic Regression Model
...
nAhead <- nrow(y_test)
oilprice_NA <-ts(c(Oil_price , rep(NA , nAhead ) ) , start = start(Oil_price) , frequency = 4)
ar_filt <- dlmFilter(oilprice_NA ,sreg_mod )
